        ...
        <div className="flex flex-col h-full">
            ...
            <ul className={isSidebarOpen ? '' : 'flex flex-col items-center'}>
              {navItems.map((item) => (
                <li key={item.name} className="mb-2">
                  <Link
                    to={item.path}
                    className={`flex items-center rounded-lg transition-colors duration-200 group ${isSidebarOpen ? 'px-4 py-3' : 'w-12 h-12 justify-center items-center'} ${isActive(item.path) ? 'text-primary font-semibold' : 'text-gray-500 hover:text-primary hover:bg-primary/10'}`}
                  >
          ...
          <div className={`py-6 space-y-2 ${isSidebarOpen ? 'px-4' : 'flex flex-col items-center'}`}>
            <button
              ...
              className={`flex items-center rounded-lg transition-colors duration-200 group ${isSidebarOpen ? 'w-full px-4 py-3 bg-primary/10 text-primary font-semibold' : 'w-12 h-12 justify-center items-center text-gray-500 hover:text-primary hover:bg-primary/10'}`}
            >
            ...
            <hr className={`border-gray-200 ${isSidebarOpen ? 'w-full' : 'w-12'}`} />
             <Link
                to="/about"
                className={`flex items-center rounded-lg transition-colors duration-200 group ${isSidebarOpen ? 'w-full px-4 py-3' : 'w-12 h-12 justify-center items-center'} ${isActive('/about') ? 'text-primary font-semibold' : 'text-gray-500 hover:text-primary hover:bg-primary/10'}`}
              >
            ...
            <button
              onClick={logout}
              className={`flex items-center rounded-lg text-gray-500 hover:text-red-600 hover:bg-red-50 transition-colors duration-200 group ${isSidebarOpen ? 'w-full px-4 py-3' : 'w-12 h-12 justify-center items-center'}`}
            >
